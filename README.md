[![CodeTime Badge](https://shields.jannchie.com/endpoint?style=for-the-badge&color=222&url=https%3A%2F%2Fapi.codetime.dev%2Fv3%2Fusers%2Fshield%3Fuid%3D32029)](https://codetime.dev)

# <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30"> Hi, I'm Natnawat

### SMART · LordEaster

**Assistant Researcher · Software Engineering · Unpaid Co-Founder at BSO Space**

<img
  align="right"
  width="180"
  src="https://github.com/user-attachments/assets/04d9a8ce-41b2-4d17-a0e4-fe0f7bc097ed"
  alt="Natnawat Panisarasirikul"
/>

Software Engineering graduate from **Burapha University, Thailand**.

Currently working as an **Assistant Researcher** and helping build products as a **Unpaid Co-Founder at [BSO Space](https://www.bsospace.com/)**.

I enjoy turning messy ideas into simple (or sometime simple into messy), useful, and maintainable software.

- Building full-stack web applications
- Designing scalable software architecture
- Exploring real-time communication systems
- Running self-hosted services
- Gaming, live streaming, and taking care of my pets

> **Be Simple but Outstanding.**

[![Website](https://img.shields.io/badge/Website-with_Yamroll-222222?style=flat-square)](https://www.withyamroll.com/)
[![BSO Space](https://img.shields.io/badge/BSO_Space-Co--Founder-222222?style=flat-square)](https://www.bsospace.com/about/natnawat)
[![YouTube](https://img.shields.io/badge/YouTube-smart__natnawat-FF0000?style=flat-square&logo=youtube&logoColor=white)](https://www.youtube.com/@withyamroll)
[![GitHub](https://img.shields.io/badge/GitHub-LordEaster-181717?style=flat-square&logo=github)](https://github.com/lordeaster)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Natnawat-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/natnawat)
[![Medium](https://img.shields.io/badge/Medium-Natnawat-000000?style=flat-square&logo=medium)](https://natnawat.medium.com/)

---

## What I'm Working On

### World Health System

A platform for exploring global health and environmental data through dashboards, maps, external data connectors, and research tools.

Currently rebuilding it from zero with a cleaner and more maintainable architecture.

`React` · `TypeScript` · `TanStack` · `Vite` · `Elysia` · `Bun` · `MongoDB` · `PostgreSQL` · `Redis` · `Docker`

### OBS Realtime Room

A self-hosted online studio for bringing multiple cameras, microphones, and screen shares into OBS with low latency.

`React` · `TypeScript` · `LiveKit` · `WebRTC` · `Bun` · `Docker` · `OBS`

---

## Learning Now

- Maintainable software architecture
- Real-time systems with WebRTC and LiveKit
- Research software and data platforms
- Self-hosted infrastructure
- Docker, Cloudflare, and service monitoring
- Better product design and developer experience

---

## Tech Stack

[![My Skills](https://skillicons.dev/icons?i=ts,js,python,bun,nodejs,react,nextjs,vite,tailwind,express,mongodb,postgres,redis,docker,nginx,cloudflare,ubuntu,github,git,figma)](https://skillicons.dev)

`Elysia` · `TanStack` · `LiveKit` · `WebRTC` · `OBS Studio` · `Tailscale`

---

## Highlights

- **IT Specialist: Cybersecurity** — Certiport / Pearson  
  [Certificate](https://drive.google.com/file/d/1LKgu3sAQsTBP-WNx5q_qLM1ry6D6Vs8L/view?usp=sharing)

- **61st out of 709 teams** — Hack The Scammer CTF 2025

- **Popular Vote Award** — Vision-Language Model for Smart Manufacturing Hackathon  
  [Certificate](https://drive.google.com/file/d/11b8_NgxEhgLsPa8i-_gbFnjgutW91apz/view?usp=sharing)

- **Google Project Management Professional Certificate**  
  [Certificate](https://www.coursera.org/account/accomplishments/specialization/42OYFFH2WJ44)

---

<details>
  <summary><strong>GitHub Stats</strong></summary>

  <br>

  ![](https://github-profile-trophy.vercel.app/?username=lordeaster&no-frame=false&no-bg=false&margin-w=4)

  ![](https://github-readme-stats.vercel.app/api?username=lordeaster&hide_border=false&include_all_commits=true&count_private=true)

  ![](https://github-readme-streak-stats.herokuapp.com/?user=lordeaster&hide_border=false)

</details>

---

![Snake animation](https://raw.githubusercontent.com/lordeaster/lordeaster/refs/heads/output/github-contribution-grid-snake.svg)
